...
export const products = mysqlTable(
  "products",
  {
    ...
    categorySource: mysqlEnum("categorySource", ["rule", "manual", "unassigned"])
      .default("unassigned")
      .notNull(),
    isPublished: boolean("isPublished").default(true).notNull(),
    isRemovedFromLatestImport: boolean("isRemovedFromLatestImport").default(false).notNull(),
    reviewStatus: mysqlEnum("reviewStatus", ["clean", "needs_review", "archived"])
      .default("clean")
      .notNull(),
    createdAt: timestamp("createdAt").defaultNow().notNull(),
    updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
...
export const importChanges = mysqlTable(
  "import_changes",
  {
    id: int("id").autoincrement().primaryKey(),
    ...
    changeType: mysqlEnum("changeType", ["new_product", "new_variant", "stock_price_update", "missing_from_import", "needs_review"])
      .notNull(),
    beforeJson: json("beforeJson"),
    afterJson: json("afterJson"),
    reviewStatus: mysqlEnum("reviewStatus", ["pending", "accepted", "ignored"])
      .default("pending")
      .notNull(),
    createdAt: timestamp("createdAt").defaultNow().notNull(),
  },
