...
function ImportChangeValues({ change }: { change: ImportChangeView }) {
  if (change.type === "new_product" || change.type === "new_variant") return <p className="import-change-new-values">{importChangeIdentity(change)} · Price {formatImportPrice(change.price)} · Quantity {change.stock ?? "—"}</p>;
  return <div className="import-change-comparisons"><p className="import-change-identity">{importChangeIdentity(change)}</p>{change.stockChanged && <p className="import-change-comparison"><span>Quantity</span><b>{change.previousStock ?? "—"}</b><i aria-label="changes to">→</i><b>{change.stock ?? "—"}</b></p>}{change.priceChanged && <p className="import-change-comparison"><span>Price</span><b>{formatImportPrice(change.previousPrice)}</b><i aria-label="changes to">→</i><b>{formatImportPrice(change.price)}</b></p>}</div>;
}

function ImportChangeGroups({ groups }: { groups: ImportChangeGroupView[] }) {
  return <div className="import-change-group-list">{groups.length ? groups.map(group => <article className="import-change-group" key={group.code}><header><div><p className="eyebrow">CLEANED-CODE ITEM</p><h4>{group.code}</h4></div><span>{group.changes.length} POS change{group.changes.length === 1 ? "" : "s"}</span></header><div className="import-variant-change-list">{group.changes.map(change => <div className={"import-variant-change-row is-" + change.type} key={change.id}><p className="eyebrow">{importChangeTitle(change)}</p><ImportChangeValues change={change} /></div>)}</div></article>) : <p className="empty-workspace">No new price or quantity changes were found in this file.</p>}</div>;
}
