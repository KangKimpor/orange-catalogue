  ...
  return (
    <div className="admin-app">
      ...
      <main className="admin-workspace">
        ...
        {workspace === "reviews" && <section className="admin-view review-view"><div className="workspace-intro"><div><h2>Inventory update review</h2><p>This list only shows imported variants whose price or stock quantity changed. Newly added and unchanged POS records do not appear here.</p></div><span className="helper-chip">{attentionCount} update{attentionCount === 1 ? "" : "s"}</span></div><section className="review-card">{reviewQueue.data?.length ? reviewQueue.data.map(change => <article key={change.id}><div><p className="eyebrow">PRICE OR STOCK CHANGE</p><h3>{change.cleanedCode}</h3><p>{change.priceChanged && `Price: ${change.previousPrice} → ${change.price}`}{change.priceChanged && change.stockChanged && " · "}{change.stockChanged && `Stock: ${change.previousStock} → ${change.stock}`}</p></div><div><button type="button" className="secondary-action" onClick={() => resolveImportChange.mutate({ id: change.id, reviewStatus: "accepted" })}>Mark reviewed</button><button type="button" className="quiet-action" onClick={() => resolveImportChange.mutate({ id: change.id, reviewStatus: "ignored" })}>Dismiss</button></div></article>) : <div className="empty-workspace">No price or stock updates need review right now.</div>}</section></section>}
