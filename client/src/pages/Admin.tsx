      ...
      <main className="admin-workspace">
        ...
        {workspace === "overview" && (
          <section className="admin-view overview-view">
            ...
            <section className="vercel-analytics-panel" aria-label="Vercel Analytics snapshot"><header><div><p className="eyebrow">VERCEL ANALYTICS</p><h3>Storefront activity</h3><p>{vercelAnalyticsSnapshot.source} · {vercelAnalyticsSnapshot.reportingPeriod}</p></div><span>Snapshot</span></header><div className="analytics-metrics"><article><span>Visitors</span><strong>{vercelAnalyticsSnapshot.visitors}</strong><small>Unique visitors</small></article><article><span>Pageviews</span><strong>{vercelAnalyticsSnapshot.pageviews}</strong><small>Tracked visits</small></article></div><div className="analytics-routes"><p className="eyebrow">TOP ROUTES</p>{vercelAnalyticsSnapshot.routes.map(route => <article key={route.path}><b>{route.path}</b><span>{route.pageviews} pageviews · {route.visitors} visitors</span></article>)}</div></section>
          </section>
        )}
