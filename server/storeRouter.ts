...
export const storeRouter = router({
  ...
  admin: router({
    ...
    updateProduct: publicProcedure.input(z.object({ id: z.number().int(), displayName: z.string().max(255).nullable(), categoryId: z.number().int().nullable(), isPublished: z.boolean().optional(), reviewStatus: z.enum(["clean", "needs_review", "archived"]).optional() })).mutation(async ({ ctx, input }) => { await requireAdmin(ctx as Context); await supabaseRequest(`products?${supabaseEq("id", input.id)}`, { method: "PATCH", body: JSON.stringify({ display_name: input.displayName, category_id: input.categoryId, category_source: input.categoryId ? "manual" : "unassigned", ...(input.isPublished === undefined ? {} : { is_published: input.isPublished }), ...(input.reviewStatus === undefined ? {} : { review_status: input.reviewStatus }) }) }); return { success: true }; }),
