...
export const storeRouter = router({
  ...
  admin: router({
    ...
    reviewQueue: publicProcedure.query(async ({ ctx }) => { await requireAdmin(ctx as Context); const rows = await supabaseRequest<Array<{ id: number; after_json: { code?: string; priceChanged?: boolean; stockChanged?: boolean; previousPrice?: number; price?: number; previousStock?: number; stock?: number } | null; review_status: string }>>("import_changes?select=id,after_json,review_status&change_type=eq.stock_price_update&review_status=eq.pending&order=created_at.desc&limit=200"); return rows.flatMap(row => { const change = row.after_json; if (!change) return []; const cleanedCode = change.code?.trim(); if (!cleanedCode || (!change.priceChanged && !change.stockChanged)) return []; return [{ id: row.id, cleanedCode, priceChanged: Boolean(change.priceChanged), stockChanged: Boolean(change.stockChanged), previousPrice: change.previousPrice ?? null, price: change.price ?? null, previousStock: change.previousStock ?? null, stock: change.stock ?? null, reviewStatus: row.review_status }]; }); }),
