...
export const storeRouter = router({
  ...
  admin: router({
    ...
    reviewQueue: publicProcedure.query(async ({ ctx }) => { await requireAdmin(ctx as Context); const rows = await supabaseRequest<Array<{ id: number; after_json: { code?: string } | null; review_status: string }>>("import_changes?select=id,after_json,review_status&change_type=eq.new_product&review_status=eq.pending&order=created_at.desc&limit=200"); const queuedCodes = new Set<string>(); return rows.flatMap(row => { const cleanedCode = row.after_json?.code?.trim(); if (!cleanedCode || queuedCodes.has(cleanedCode)) return []; queuedCodes.add(cleanedCode); return [{ id: row.id, cleanedCode, reviewStatus: row.review_status }]; }); }),
