-- Retain the exact normalized POS rows for every confirmed import so selected imports can be
-- removed and the catalogue can be rebuilt deterministically from the remaining snapshots.
alter table public.imports
  add column if not exists source_items_json jsonb;
...
begin
  ...
  update public.variants set last_seen_import_id = null;
