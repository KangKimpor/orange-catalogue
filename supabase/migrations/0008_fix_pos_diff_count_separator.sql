-- Hotfix the detailed POS diff counter in the deployed traceability procedure.
-- PostgreSQL text cannot contain chr(0), so use composite DISTINCT values instead of
-- concatenating fields with a null-character separator.
...
begin
  ...
  with incoming as (
    ...
    select count(distinct (cleaned_code, color_key)) filter (where stored_change_type = 'new_color') as new_colors,
           count(distinct (cleaned_code, color_key, coalesce(size, ''))) filter (where stored_change_type = 'new_size') as new_sizes,
